...
class AutopilotController:
    ...
    def __init__(self, network_interface, model_path):
        ...
        self.model = RobopilotMLP(dropout_rate=0.0)
    ...
    def process_frame(self, snapshot):
        ...
        actions = {
            "forward": predictions[0][0].item() > 0.5,
...
class WorkConsole(QMainWindow):
    def __init__(self, model_path_from_args):
        super().__init__()
        loadUi(os.path.join(os.path.dirname(__file__), "Console.ui"), self)
    ...
    def update_ui_state(self):
        ...
        self.saveRecordButton.setEnabled(len(self.recorded_data) > 0 and not self.recording)
    ...
    def save_record(self):
        if self.saving_worker is not None or len(self.recorded_data) == 0: return
        if self.recording: self.toggle_record()

        self.saveRecordButton.setText("Saving...")
        record_name = "record_%d.npz"
        fid = 0
        while os.path.exists(record_name % fid): fid += 1
